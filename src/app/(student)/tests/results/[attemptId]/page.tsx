...
export default async function ResultsPage({
  ...
  const { attempt, answers, categoryScores, totalQuestions, score, percentage, passed } =
    ...
    <div className="mx-auto max-w-3xl space-y-6">
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="grid grid-cols-3 gap-4 border-t pt-6 text-center">
            <div>
              ...
              <p className="mt-1 font-semibold">{score} / {totalQuestions}</p>
            </div>
            <div>
              <div className="flex items-center justify-center gap-1.5 text-muted-foreground">
              ...
              <p className="mt-1 font-semibold">{formatTime(attempt.time_taken_seconds)}</p>
            </div>
            <div>
              <div className="flex items-center justify-center gap-1.5 text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {categoryScores.map((cat) => {
              const catPercentage = cat.total > 0 ? Math.round((cat.correct / cat.total) * 100) : 0
              return (
      ...
      <div className="space-y-4">
        ...
        {answers.map((answer, i) => {
          ...
          return (
            <Card key={answer.id} className={cn('border-l-4', isCorrect ? 'border-l-green-500' : 'border-l-red-500')}>
              <CardContent className="space-y-3 pt-6">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isStudentAnswer && !isCorrectAnswer && 'border-red-500 bg-red-50 dark:bg-red-950/30'
                        )}
                      >
                        <span className="mr-2 font-semibold uppercase">{letter}.</span>
                        {text}
                        {isStudentAnswer && (
                ...
                {(q.explanation || q.grammar_notes || q.vocabulary_meaning || q.kanji_reading) && (
                  <div className="space-y-1 border-t pt-3 text-sm">
                    {q.explanation && (
                      <p><span className="font-medium">Explanation: </span>{q.explanation}</p>
                    )}
                    {q.grammar_notes && (
                      <p><span className="font-medium">Grammar notes: </span>{q.grammar_notes}</p>
                    )}
                    {q.vocabulary_meaning && (
                      <p><span className="font-medium">Vocabulary: </span>{q.vocabulary_meaning}</p>
                    )}
                    {q.kanji_reading && (
                      <p><span className="font-medium">Kanji reading: </span>{q.kanji_reading}</p>
                    )}
                  </div>
                )}
              </CardContent>
